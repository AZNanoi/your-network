...
<?php 
  ...
  if (isset($_SESSION["fb-userName"])){
    ...
    $response=$facebook->api('/me?fields=name,posts.offset(1){created_time,id,message,story,link,full_picture,attachments{media,subattachments{media}}}', 'GET');
...
";
    ...
    <?php
      ...
      require("dbc.php");
      $strid=strval($user);
      $query="SELECT * FROM posts WHERE uid='$strid' ORDER BY created_time DESC";
      if (($result=mysqli_query($link, $query)) === false) {
        printf("Query failed: %s <br />\n%s", $query, mysqli_error($link));
        exit();
      }
      $db_response=mysqli_fetch_all($result,MYSQLI_ASSOC);
      $m_response=array_merge($r_posts,$db_response);
      ...
      $lpi_index=index_search($serialized, "id", $last_post_id);
      $eNum=count($serialized);
      $diff=$eNum-$lpi_index;
      if($diff != 0){
        if($lpi_index+1 == $eNum){
          $serialized=array_slice($serialized,$lpi_index+1);
        }else{
          $serialized=array_slice($serialized,$lpi_index+1,2);
        }
